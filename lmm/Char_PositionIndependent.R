...
length(as.list(unique(raw$item)))#4225
...
##check whether we need to log it or not
plot(raw$CHRCount,raw$zscore)
plot(raw$absolute_freq_sd_log,raw$zscore)

plot(raw$rad_freq_np,raw$zscore)
plot(raw$nradical_frequency_log,raw$zscore)
#charnpLogCharFreqvsZscore

plot(raw$res_freq_np,raw$zscore)
# rad log, res no log

#### build initial model
m0 <- lmer(zscore ~  absolute_freq_sd_log * nradical_frequency_log  * res_freq_np + (1|item) +(1|subject),data=raw,)
summary(m0)
...
#### Refit the model 
m1 <- lmer(zscore ~  absolute_freq_sd_log * nradical_frequency_log  * res_freq_np + (1|item) +(1|subject),data=r,REML=FALSE)
...
m3 <- ffRanefLMER.fnc(model = m2, list(slopes = c("absolute_freq_sd_log",
                                                  "nradical_frequency_log",
                                                  "res_freq_np"), corr = c(0, 0, 0), by.vars = c("subject","item")))
...
## plot 
plot_interaction(r,m4)#2.13  8.31 14.49
...
plot_interaction <- function(r_filtered, m6) {
    ...
    mean_rad <- round(mean(r_filtered$nradical_frequency_log),2)#1.9
    sd_rad <- round(sd(r_filtered$nradical_frequency_log),2)#.6
    
    mean_res <- round(mean(r_filtered$res_freq_np),2)#.9
    sd_res <- round(sd(r_filtered$res_freq_np),2)#.3
    
    low_freq <- mean_freq-sd_freq
    high_freq <- mean_freq+sd_freq

    low_rad <- mean_rad-sd_rad
    high_rad <- mean_rad+sd_rad
    low_res <- mean_res-sd_res
    high_res <- mean_res+sd_res
    ...
    g1 <- ggplot(e.df,aes(x=nradical_frequency_log,y=fit,group=res_freq_np,col=res_freq_np,ymin=lower,ymax=upper)) + 
      geom_line(size = 1)+
      ...
      scale_x_continuous(breaks = c(1.5, 2.0, 2.5)) +
      xlab("Radical Frequency (log10)") + ylab("Predicted zRTs") + ggtitle("Three-way Interaction Effects (Position-Independent)")
    return (g1)}
